 ...
 *
 * SEGURIDAD (decisión del owner): es abierto por email a propósito — cero
 * fricción. El acceso físico real es huella + recepción, y la clave temporal se
 * cambia forzosamente al entrar. No hay código. (El escenario de "tomar" una
 * cuenta dormida sabiendo el correo se acepta a propósito para este contexto:
 * gym chico, sin tarjeta en cuentas que nunca iniciaron sesión.)
 */
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (fichaPrevia?.auth_id) {
      // Cuenta del flujo viejo (login con Cambiar123 al dar de alta) que NUNCA se
      // usó: como /activar decía "ya activa", el socio nunca veía su clave temporal
      // y quedaba atorado. Si NUNCA inició sesión, le re-mostramos la temporal
      // (Cambiar123 es pública: no expone nada) y la app le fuerza el cambio al
      // entrar. Si ya entró alguna vez, NO tocamos su clave.
