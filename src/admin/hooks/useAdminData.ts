...
// pago_unico es columna nueva; los tipos generados de Supabase todavía no la
// tienen, así que se agrega a mano y se castea en la llamada.
export async function updateTier(
  tierId: string,
  patch: Partial<Pick<Tier, 'nombre' | 'descripcion' | 'precio_centavos' | 'inscripcion_centavos' | 'invitados_por_periodo' | 'moneda' | 'periodo' | 'tipo' | 'clases_incluidas' | 'duracion_dias' | 'beneficios' | 'reglas' | 'activo' | 'orden' | 'slug' | 'acceso_todas_sucursales'>> & { pago_unico?: boolean; es_pase?: boolean; vigencia_inicio?: string | null; vigencia_fin?: string | null; dias_acceso?: number[] | null; max_reservas_dia?: number | null; en_venta?: boolean; visible_landing?: boolean }
): Promise<{ error: string | null }> {
...
export async function insertTier(
  payload: Database['public']['Tables']['tiers']['Insert'] & { pago_unico?: boolean; es_pase?: boolean; vigencia_inicio?: string | null; vigencia_fin?: string | null; dias_acceso?: number[] | null; max_reservas_dia?: number | null; en_venta?: boolean; visible_landing?: boolean }
