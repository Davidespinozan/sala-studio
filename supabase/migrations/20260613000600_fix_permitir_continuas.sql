-- ============================================================================
-- FIX M3 — reservar_clase_atomic ignoraba config.reserva.permitir_continuas.
-- ----------------------------------------------------------------------------
-- AjustesReglas guarda el toggle "Permitir reservas continuas", pero la función
-- bloqueaba SIEMPRE con CONTINUA sin leer la config → el admin lo activaba y no
-- tenía efecto. Hermano del bug #3 (clave de config que se escribe pero no se
-- lee). Recreamos reservar_clase_atomic VERBATIM de 20260613000200 (que ya trae
-- el fix de anticipación) condicionando el bloqueo de continuas al toggle.
-- ============================================================================
...
COMMENT ON FUNCTION reservar_clase_atomic(uuid, integer, text) IS
  'Reserva una clase por clase_id. Gate de membresía (socios): SIN_MEMBRESIA / MEMBRESIA_CONGELADA / MEMBRESIA_VENCIDA / SIN_CREDITOS. Débito post-cupo atómico con FOR UPDATE. Admin/recepcionista/staff bypassean el gate. Anticipación: config.reserva.anticipacion_min_horas (fix #3). Continuas: bloqueadas salvo config.reserva.permitir_continuas=true (fix M3). Folio SAL-NNNNNN. tz de la sucursal.';

-- ============================================================================
-- TEST — la lectura de permitir_continuas toma la clave que el admin escribe.
-- Mirror del COALESCE de la función: con permitir_continuas=true la expresión
-- resuelve true (default sin setear = false). Savepoint + centinela.
-- ============================================================================
DO $$
DECLARE
  v_tenant uuid;
...
BEGIN
  ...
  BEGIN
    -- default (sin la clave) = false
    UPDATE tenants SET config = (COALESCE(config,'{}'::jsonb) #- '{reserva,permitir_continuas}') WHERE id = v_tenant;
    SELECT COALESCE((config->'reserva'->>'permitir_continuas')::boolean, false) INTO v_default
    FROM tenants WHERE id = v_tenant;
    IF v_default <> false THEN
      RAISE EXCEPTION 'TEST FALLO: default de permitir_continuas no es false (%).', v_default;
    END IF;

    -- seteado en true
    UPDATE tenants SET config = jsonb_set(COALESCE(config,'{}'::jsonb), '{reserva,permitir_continuas}', 'true'::jsonb, true) WHERE id = v_tenant;
    SELECT COALESCE((config->'reserva'->>'permitir_continuas')::boolean, false) INTO v_leido
    FROM tenants WHERE id = v_tenant;
    IF v_leido <> true THEN
      RAISE EXCEPTION 'TEST FALLO: permitir_continuas=true se leyó % (esperaba true).', v_leido;
    END IF;

    RAISE EXCEPTION 'ROLLBACK_FIX_CONT';
  EXCEPTION WHEN raise_exception THEN
    IF SQLERRM = 'ROLLBACK_FIX_CONT' THEN NULL;
    ELSE RAISE; END IF;
  END;

  RAISE NOTICE 'TEST OK: permitir_continuas se lee de config.reserva (default false, settable a true). Revertido.';
END $$;
