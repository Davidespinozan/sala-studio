-- ============================================================================
-- FIX (nada hardcodeado) — reservar_clase_atomic: max invitados del tier.
-- ----------------------------------------------------------------------------
-- El gate usaba max_invitados_por_tier(slug) con CASE hardcoded (basica=2,pro=4,
-- tenant-agnóstico). Ya hubo un fix (20260517000001) que leía tiers.reglas, pero
-- 20260524200000 lo regresó. Recreamos el gate vivo (20260613000600) leyendo
-- tiers.reglas->>'max_invitados' por tenant (editable por el admin en Planes),
-- con fallback a la función legacy. Un tenant con slugs propios deja de quedar
-- con 0 invitados.
-- ============================================================================
...
COMMENT ON FUNCTION reservar_clase_atomic(uuid, integer, text) IS
  'Reserva una clase por clase_id. Gate de membresía + débito atómico. Anticipación: config.reserva.anticipacion_min_horas. Continuas: config.reserva.permitir_continuas. Max invitados: tiers.reglas->>max_invitados (fallback legacy). Admin/recep/staff bypassean el gate.';

-- ============================================================================
-- TEST (smoke) — el gate lee max_invitados de tiers.reglas.
-- ============================================================================
DO $$
DECLARE v_def text;
BEGIN
  SELECT pg_get_functiondef('reservar_clase_atomic(uuid, integer, text)'::regprocedure) INTO v_def;
  IF position('reglas->>''max_invitados''' in v_def) = 0 THEN
    RAISE EXCEPTION 'TEST FALLO: el gate no lee max_invitados de tiers.reglas.';
  END IF;
  RAISE NOTICE 'TEST OK: el gate lee max_invitados de tiers.reglas (nada hardcodeado).';
END $$;
