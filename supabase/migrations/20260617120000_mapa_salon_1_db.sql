-- ============================================================================
-- MAPA DE SALÓN · Lote 1 — base de datos + RPC de reserva con lugar
-- ----------------------------------------------------------------------------
-- Cada sala (recurso) puede tener un LAYOUT opcional: un mapa de lugares que el
-- socio elige al reservar. Sin layout → todo sigue por cupo/conteo como hoy
-- (no rompe nada).
--
--   recursos.layout  jsonb  → diseño de la sala (lugares + elementos + ícono).
--     { "tipo_icono": "bici|mat|reformer|generico",
--       "lugares":   [ {"id":"L1","label":"1","x":0,"y":0}, ... ],
--       "elementos": [ {"tipo":"instructor","x":3,"y":0}, ... ] }
--   reservas.lugar_id text   → lugar elegido (ej 'L3'); NULL si la sala no usa mapa.
--
-- Un índice único parcial impide doble-reservar un lugar en la misma clase
-- (resuelve la carrera en DB, igual que el viejo unique de slot).
-- ============================================================================

ALTER TABLE recursos  ADD COLUMN IF NOT EXISTS layout jsonb;
ALTER TABLE reservas  ADD COLUMN IF NOT EXISTS lugar_id text;

COMMENT ON COLUMN recursos.layout IS
  'Mapa de salón (opcional): lugares reservables + elementos decorativos + ícono por disciplina. NULL = sala sin mapa (cupo por conteo).';
COMMENT ON COLUMN reservas.lugar_id IS
  'Lugar elegido dentro del layout de la sala (id del lugar). NULL si la sala no usa mapa.';

-- Un lugar, una reserva activa por clase.
CREATE UNIQUE INDEX IF NOT EXISTS reservas_lugar_unico
  ON reservas (clase_id, lugar_id)
  WHERE status IN ('confirmada', 'completada') AND lugar_id IS NOT NULL;

-- ----------------------------------------------------------------------------
-- RPC: reservar_clase_atomic + p_lugar_id
-- ----------------------------------------------------------------------------
-- Se reemplaza la firma (3 → 4 args): se dropea la vieja y se crea la nueva con
-- p_lugar_id DEFAULT NULL (las llamadas actuales por nombre, sin lugar, siguen
-- funcionando). Cambios vs versión anterior:
--   - Si la sala tiene layout: lugar requerido + validado + libre; sin invitados
--     (cada persona reserva su propio lugar); el cupo efectivo = nº de lugares.
--   - Si no tiene layout: ignora p_lugar_id; todo igual que antes.
DROP FUNCTION IF EXISTS reservar_clase_atomic(uuid, integer, text);
