...
-- NO reservar clases DESPUÉS de que vence el plan.
-- ────────────────────────────────────────────────────────────────────────────
-- Bug reportado: un socio que vence MAÑANA podía reservar toda la semana. El gate
-- solo bloqueaba si la membresía YA estaba vencida hoy (periodo_actual_fin <= now),
-- pero no impedía reservar una clase FUTURA que cae después del vencimiento.
--
-- Se re-crea reservar_clase_atomic (donde vive el gate; reservar_clase_virtual y
-- los wrappers _con_multa DELEGAN en ella) para agregar el check: un socio no puede
-- reservar una clase cuyo DÍA sea posterior a su día de vencimiento. Se compara por
-- FECHA en la tz del gym, así una clase del MISMO día que vence SÍ se permite.
-- Admin/recepción (v_es_socio = false) siguen sin gate, como el resto de reglas.
-- El cuerpo es IDÉNTICO al vivo (20260815220000) + el bloque nuevo tras calcular el
-- horario de la clase. GRANT/REVOKE se conservan (CREATE OR REPLACE no los toca).
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- VIGENCIA: un socio no puede reservar una clase cuyo DÍA sea posterior a su día
  -- de vencimiento. Se compara por FECHA en la tz del gym: una clase del MISMO día
  -- que vence SÍ se permite; los días siguientes NO. (El gate de arriba solo cubre
  -- "ya venció hoy"; esto cubre reservas FUTURAS fuera de vigencia.)
  IF v_es_socio AND v_mem_fin IS NOT NULL
...
-- ════════════════════════════════════════════════════════════════════════════
-- TEST — devuelve TABLA: la función tiene el check y la matemática de fecha es
-- correcta (el día siguiente al vencimiento se bloquea; el MISMO día se permite).
-- ════════════════════════════════════════════════════════════════════════════
SELECT
  'reserva dentro de vigencia' AS prueba,
  pg_get_functiondef('reservar_clase_atomic(uuid, integer, text, text)'::regprocedure)
    ILIKE '%CLASE_FUERA_DE_VIGENCIA%'                 AS tiene_check,
  (date '2026-10-24' > date '2026-10-23')             AS dia_siguiente_bloquea,
  (NOT (date '2026-10-23' > date '2026-10-23'))       AS mismo_dia_permite;
